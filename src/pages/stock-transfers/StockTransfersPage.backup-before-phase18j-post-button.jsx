...
export default function StockTransfersPage() {
  ...
  const updateTransferStatus = async (transfer, nextStatus) => {
    ...
    try {
      ...
      const message = `${response?.data?.transferCode || transfer.transferCode} is now ${nextStatus.toLowerCase()}.`
      await loadTransfers()
          ...
          <div className="table-wrapper">
            <table className="min-w-[920px] w-full text-left text-sm">
              ...
              <tbody className="divide-y divide-[var(--color-border)] bg-white">
                {isLoading ? (
                  <tr>
                    <td className="px-4 py-8 text-center font-bold text-[var(--color-muted)]" colSpan={8}>
                        ...
                        <td className="px-4 py-4">
                          {transfer.status === "REQUESTED" ? (
                            <div className="flex flex-col gap-2">
                              <button
                                className="rounded-2xl bg-[#7A1F2B] px-4 py-2 text-xs font-black text-white transition hover:bg-[#641824] disabled:cursor-not-allowed disabled:opacity-60"
                                disabled={actionTransferId === transfer.id}
                                onClick={() => updateTransferStatus(transfer, "APPROVED")}
                                type="button"
                              >
                                {actionTransferId === transfer.id ? "Saving..." : "Approve"}
                              </button>
                              <button
                                className="rounded-2xl border border-rose-300 bg-white px-4 py-2 text-xs font-black text-rose-700 transition hover:bg-rose-50 disabled:cursor-not-allowed disabled:opacity-60"
                                disabled={actionTransferId === transfer.id}
                                onClick={() => updateTransferStatus(transfer, "REJECTED")}
                                type="button"
                              >
                                Reject
                              </button>
                            ...
                            <span className="text-xs font-bold text-[var(--color-muted)]">No action</span>
                          )}
                        </td>
                      </tr>
                    ))
