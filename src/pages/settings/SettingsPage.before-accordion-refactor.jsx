...
import { updateIncentiveRules } from "../../features/incentives/incentives.api"
import EnterpriseIncentiveSettings from "../../features/incentives/EnterpriseIncentiveSettings"
import {
  findSettingByKey,
  formatReadableText,
...
function IncentiveRulesForm({ setting, onSaved, canManageIncentives = false }) {
  ...
  useEffect(() => {
    const nextRules = setting?.value && typeof setting.value === "object" ? setting.value : {}
    // Keep the editable draft aligned with the latest saved setting.
    // eslint-disable-next-line react-hooks/set-state-in-effect
    setRules(nextRules)
  }, [setting])
  ...
  const toggle = (key) => {
    if (!canManageIncentives || isSaving) return
    setRules((current) => ({ ...current, [key]: !current[key] }))
  }

  const reset = () => {
    setRules(setting.value && typeof setting.value === "object" ? setting.value : {})
    setMessage("")
    setErrorMessage("")
  }
  ...
  const save = async (event) => {
    event.preventDefault()
    if (!canManageIncentives) return
    setMessage("")
    setErrorMessage("")

    const savedRules = setting.value && typeof setting.value === "object" ? setting.value : {}

    const nextValue = {
      enableItemIncentives: Boolean(rules.enableItemIncentives),
      enableServiceIncentives: Boolean(rules.enableServiceIncentives),
      // Retained only so the legacy response contract remains stable. New
      // postings obtain rates exclusively from immutable enterprise versions.
      defaultItemIncentivePercent: Number(savedRules.defaultItemIncentivePercent || 0),
      defaultServiceIncentivePercent: Number(savedRules.defaultServiceIncentivePercent || 0),
      staffCanViewOwnIncentives: Boolean(rules.staffCanViewOwnIncentives),
      ownerCanViewAllIncentives: Boolean(rules.ownerCanViewAllIncentives),
      requireOwnerApprovalBeforePayout: Boolean(savedRules.requireOwnerApprovalBeforePayout),
    }

    if (JSON.stringify(nextValue) === JSON.stringify(setting.value)) {
      setMessage("No changes to save.")
      return
    }
    ...
    setIsSaving(true)
    try {
      const response = await updateIncentiveRules(nextValue)
      if (!response?.success || !response?.data?.rules) throw new Error("Unable to save incentive rules.")
      const savedSetting = {
        ...setting,
        value: nextValue,
        updatedAt: response.data.rules.updatedAt || setting.updatedAt,
      }
      onSaved(savedSetting)
      setMessage("Incentive posting and visibility rules saved. Rates come from the immutable matrix below.")
    } catch (error) {
      setErrorMessage(
        error?.response?.data?.error?.message ||
          error?.response?.data?.message ||
          error?.message ||
          "Unable to save incentive rules.",
      )
    } finally {
      setIsSaving(false)
    }
  ...
  const toggles = [
    ["enableItemIncentives", "Enable product incentives"],
    ["enableServiceIncentives", "Enable service incentives"],
    ["staffCanViewOwnIncentives", "Staff can view their own incentives"],
    ["ownerCanViewAllIncentives", "Owners and admins can monitor all incentives"],
  ]
  ...
  return (
    <Card className="border-[var(--color-maroon)] ring-2 ring-[var(--color-maroon-soft)]">
      <div className="flex flex-col gap-3 xl:flex-row xl:items-start xl:justify-between">
        <div className="min-w-0">
          <Badge tone="maroon">{canManageIncentives ? "Editable" : "Viewing only"}</Badge>
          <h2 className="mt-3 text-xl font-bold text-[var(--color-text-strong)]">
            Incentive Rules
          </h2>
          <p className="mt-2 text-sm leading-6 text-[var(--color-muted)]">
            Configure which source types can earn incentives and who can view them. Compensation rates are managed in the immutable matrix below; every submitted claim follows the audited approval step before it can be marked paid.
          </p>
        </div>

        <div className="rounded-2xl bg-[var(--color-soft)] px-4 py-3 text-sm font-semibold text-[var(--color-muted)]">
          {canManageIncentives ? "Source-linked rules" : "Viewing only"}
        </div>
      </div>

      <div className="mt-5 rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm font-semibold leading-6 text-amber-800">
        Product and service toggles apply to new source transactions. The legacy percentage fields remain stored for API compatibility but are not editable or used for new postings.
      </div>

      <form className="mt-5 space-y-4" onSubmit={save}>
        <div className="grid gap-3 sm:grid-cols-2">
          {toggles.map(([key, label]) => (
            <button className="flex items-center gap-3 rounded-2xl border border-[var(--color-border)] bg-[var(--color-soft)] p-4 text-left disabled:cursor-not-allowed disabled:opacity-75" disabled={!canManageIncentives || isSaving} key={key} onClick={() => toggle(key)} type="button">
              <span className={`grid size-5 shrink-0 place-items-center rounded-md border ${rules[key] ? "border-[var(--color-maroon)] bg-[var(--color-maroon)]" : "border-[var(--color-border)] bg-white"}`}>{rules[key] ? <span className="size-2 rounded-full bg-white" /> : null}</span>
              <span className="text-sm font-bold text-[var(--color-text-strong)]">{label}</span>
            </button>
          ))}
        </div>

        {message ? <div className="rounded-2xl border border-green-200 bg-green-50 px-4 py-3 text-sm font-semibold text-green-700">{message}</div> : null}
        {errorMessage ? <div className="rounded-2xl border border-red-200 bg-red-50 px-4 py-3 text-sm font-semibold text-red-700">{errorMessage}</div> : null}

        <div className="flex flex-col gap-3 sm:flex-row">
          <button className="h-12 rounded-2xl bg-[var(--color-maroon)] px-5 text-sm font-bold text-white disabled:cursor-not-allowed disabled:opacity-60" disabled={!canManageIncentives || isSaving} type="submit">{!canManageIncentives ? "View Only" : isSaving ? "Saving..." : "Save Incentive Rules"}</button>
          <button className="h-12 rounded-2xl border border-[var(--color-border)] bg-white px-5 text-sm font-bold" disabled={isSaving} onClick={reset} type="button">Reset</button>
        </div>
      </form>
    </Card>
  )
    ...
    <Card className="border-[var(--color-maroon)] ring-2 ring-[var(--color-maroon-soft)]">
      ...
      <div className="mt-5 grid gap-3 xl:grid-cols-2">
        <div className="rounded-2xl border border-[var(--color-border)] bg-[var(--color-soft)] p-4">
          ...
          <p className="mt-2 text-sm leading-6 text-[var(--color-muted)]">
            Display only for now. This will not be editable until the related business flow is fully confirmed.
          </p>
        </div>
...
function SettingsPage({ user }) {
  ...
  const canManageIncentives = user?.role === "SUPER_OWNER"
  ...
  const incentiveRulesSetting = useMemo(
    () => findSettingByKey(settings, "incentive.rules"),
    [settings],
  )
  ...
  return (
    <div className="space-y-5">
      ...
      <IncentiveRulesForm
        canManageIncentives={canManageIncentives}
        onSaved={handleSettingSaved}
        setting={incentiveRulesSetting}
      />
      <EnterpriseIncentiveSettings canManage={canManageIncentives} />
      <CashBoxRulesForm
