...
function IncentiveRulesDisplay({ setting }) {
  if (!setting) return null

  const rules = setting.value && typeof setting.value === "object" ? setting.value : {}

  const rows = [
    {
      label: "Item incentives",
      value: rules.enableItemIncentives ? "Enabled" : "Disabled",
    },
    {
      label: "Service incentives",
      value: rules.enableServiceIncentives ? "Enabled" : "Disabled",
    },
    {
      label: "Default item incentive percent",
      value: `${Number(rules.defaultItemIncentivePercent || 0)}%`,
    },
    {
      label: "Default service incentive percent",
      value: `${Number(rules.defaultServiceIncentivePercent || 0)}%`,
    },
    {
      label: "Staff can view own incentives",
      value: rules.staffCanViewOwnIncentives ? "Yes" : "No",
    },
    {
      label: "Owner can view all incentives",
      value: rules.ownerCanViewAllIncentives ? "Yes" : "No",
    },
    {
      label: "Owner approval before payout",
      value: rules.requireOwnerApprovalBeforePayout ? "Required" : "Not required",
    },
  ]
    ...
    <Card className="border-[var(--color-maroon)] ring-2 ring-[var(--color-maroon-soft)]">
      <div className="flex flex-col gap-3 xl:flex-row xl:items-start xl:justify-between">
        <div className="min-w-0">
          ...
          <p className="mt-2 text-sm leading-6 text-[var(--color-muted)]">
            This shows saved incentive settings for the future incentive module.
          </p>
        </div>
      ...
      <div className="mt-5 rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm font-semibold leading-6 text-amber-800">
        Incentive computation is not active yet. These settings do not calculate or release payouts.
      </div>
      ...
      <div className="mt-5 grid gap-3 xl:grid-cols-2">
        {rows.map((row) => (
          <div
            ...
            key={row.label}
          >
            <p className="text-sm font-bold text-[var(--color-muted)]">{row.label}</p>
            <p className="mt-2 text-base font-bold text-[var(--color-text-strong)]">
              {row.value}
            </p>
          </div>
        ))}
...
function SettingsPage({ user }) {
  ...
  return (
    <div className="space-y-5">
      ...
      <IncentiveRulesDisplay setting={incentiveRulesSetting} />
      <CashBoxRulesForm
