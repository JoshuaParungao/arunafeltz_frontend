...
function Status({ value }) {
  const classes = value === "RECEIVED" ? "bg-emerald-50 text-emerald-700" : value === "CANCELLED" ? "bg-rose-50 text-rose-700" : value === "DRAFT" ? "bg-slate-100 text-slate-700" : "bg-amber-50 text-amber-700"
  return <span className={`rounded-full px-2.5 py-1 text-xs font-black ${classes}`}>{String(value || "—").replaceAll("_", " ")}</span>
}
...
export default function PurchaseOrdersPage({ selectedBranch, user, onNavigate }) {
  ...
  return (
    <div className="space-y-5">
      <section className="rounded-3xl border bg-white p-5 shadow-card sm:p-6"><div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-xs font-black uppercase tracking-[0.2em] text-[var(--color-maroon)]">Supply chain</p><h1 className="mt-2 text-2xl font-black">Purchase Orders</h1><p className="mt-1 text-sm text-[var(--color-muted)]">Create costed drafts, order them deliberately, and track receiving progress.</p></div><button className="inline-flex items-center justify-center gap-2 rounded-xl bg-[var(--color-maroon)] px-4 py-3 text-sm font-bold text-white" onClick={() => setEditing({})} type="button"><Plus size={17} />New PO</button></div></section>
      ...
      <section className="grid gap-3 rounded-3xl border bg-white p-4 shadow-card sm:grid-cols-[1fr_240px]"><label className="relative"><Search className="absolute left-3 top-1/2 -translate-y-1/2 text-[var(--color-muted)]" size={16} /><input className="w-full rounded-xl border py-3 pl-10 pr-3 text-sm" onChange={(event) => { setSearch(event.target.value); setPage(1) }} placeholder="PO code, supplier, notes" value={search} /></label><select className="rounded-xl border px-3 py-3 text-sm" onChange={(event) => { setStatus(event.target.value); setPage(1) }} value={status}><option value="">All statuses</option>{["DRAFT", "ORDERED", "PARTIALLY_RECEIVED", "RECEIVED", "CANCELLED"].map((value) => <option key={value} value={value}>{value.replaceAll("_", " ")}</option>)}</select></section>
      <section className="overflow-hidden rounded-3xl border bg-white shadow-card">{isLoading ? <div className="flex items-center justify-center gap-2 p-10"><LoaderCircle className="animate-spin" size={18} />Loading purchase orders...</div> : orders.length === 0 ? <div className="p-10 text-center"><ClipboardList className="mx-auto text-[var(--color-muted)]" size={40} /><p className="mt-3 font-black">No matching purchase orders</p></div> : <><div className="hidden overflow-x-auto lg:block"><table className="w-full min-w-[960px] text-left text-sm"><thead className="bg-[var(--color-soft)] text-xs uppercase text-[var(--color-muted)]"><tr><th className="px-4 py-3">PO / date</th><th className="px-4 py-3">Supplier</th><th className="px-4 py-3">Expected</th><th className="px-4 py-3">Status</th><th className="px-4 py-3 text-right">Total</th><th className="px-4 py-3 text-right">Actions</th></tr></thead><tbody className="divide-y">{orders.map((order) => <tr key={order.id}><td className="px-4 py-4"><p className="font-black">{order.poCode}</p><p className="text-xs text-[var(--color-muted)]">{dateOnly(order.orderDate)} · {order.branch?.code}</p></td><td className="px-4 py-4">{order.supplierNameSnapshot || order.supplier?.name}</td><td className="px-4 py-4">{dateOnly(order.expectedDate)}</td><td className="px-4 py-4"><Status value={order.status} /></td><td className="px-4 py-4 text-right font-black">{money(order.grandTotal)}</td><td className="px-4 py-4"><div className="flex justify-end gap-2"><button className="rounded-xl border p-2" onClick={() => openDetail(order)} type="button"><Eye size={16} /></button>{order.status === "DRAFT" ? <button className="rounded-xl border px-3 py-2 text-xs font-bold" onClick={() => setEditing(order)} type="button">Edit</button> : null}{order.status === "DRAFT" ? <button className="rounded-xl bg-emerald-600 px-3 py-2 text-xs font-bold text-white" disabled={isSaving} onClick={() => changeStatus(order, "ORDERED")} type="button">Order</button> : null}{order.status === "DRAFT" ? <button className="rounded-xl border border-rose-200 px-3 py-2 text-xs font-bold text-rose-700" disabled={isSaving} onClick={() => changeStatus(order, "CANCELLED")} type="button">Cancel</button> : null}</div></td></tr>)}</tbody></table></div><div className="grid gap-3 p-4 lg:hidden">{orders.map((order) => <article className="rounded-2xl border border-[var(--color-border)] p-4 shadow-sm" key={order.id}><div className="flex items-start justify-between gap-2"><div className="min-w-0"><p className="font-black text-[var(--color-text-strong)]">{order.poCode}</p><p className="mt-1 text-sm text-[var(--color-muted)] truncate">{order.supplierNameSnapshot || order.supplier?.name}</p></div><Status value={order.status} /></div><div className="mt-3 flex items-end justify-between border-t border-[var(--color-border)] pt-3"><div><p className="text-xs text-[var(--color-muted)]">Order date</p><p className="font-bold">{dateOnly(order.orderDate)}</p></div><div className="text-right"><p className="text-xs text-[var(--color-muted)]">Grand total</p><p className="font-black text-[var(--color-maroon)]">{money(order.grandTotal)}</p></div></div><div className="mt-4 flex flex-wrap gap-2"><button className="flex-1 min-w-[70px] rounded-xl border border-[var(--color-border)] bg-white px-3 py-2 text-xs font-bold transition hover:bg-[var(--color-soft)]" onClick={() => openDetail(order)} type="button">View PO</button>{order.status === "DRAFT" ? <><button className="rounded-xl border border-[var(--color-border)] bg-white px-3 py-2 text-xs font-bold transition hover:bg-[var(--color-soft)]" onClick={() => setEditing(order)} type="button">Edit</button><button className="rounded-xl bg-emerald-600 px-3 py-2 text-xs font-bold text-white shadow-sm transition hover:bg-emerald-700 disabled:opacity-50" disabled={isSaving} onClick={() => changeStatus(order, "ORDERED")} type="button">Order</button><button className="rounded-xl border border-rose-200 bg-rose-50 px-3 py-2 text-xs font-bold text-rose-700 transition hover:bg-rose-100 disabled:opacity-50" disabled={isSaving} onClick={() => changeStatus(order, "CANCELLED")} type="button">Cancel</button></> : null}{["ORDERED", "PARTIALLY_RECEIVED"].includes(order.status) && onNavigate ? <button className="rounded-xl bg-[var(--color-maroon)] px-3 py-2 text-xs font-bold text-white shadow-sm" onClick={() => onNavigate("receivings")} type="button">Receive</button> : null}</div></article>)}</div></>}
      ...
      {editing ? <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-950/55 p-3 sm:p-6"><section className="mx-auto max-w-6xl rounded-3xl bg-white p-5 shadow-2xl"><div className="mb-5 flex justify-between"><h2 className="text-xl font-black">{editing.id ? `Edit ${editing.poCode}` : "New purchase order"}</h2><button className="rounded-xl border p-2" onClick={() => setEditing(null)} type="button"><X size={18} /></button></div><PurchaseOrderForm catalogItems={catalogItems} initial={editing} isSaving={isSaving} onClose={() => setEditing(null)} onSave={save} suppliers={suppliers} /></section></div> : null}
      {detail ? <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-950/55 p-3 sm:p-6"><section className="mx-auto max-w-5xl rounded-3xl bg-white shadow-2xl"><header className="flex items-start justify-between border-b p-5"><div><p className="text-xs font-black text-[var(--color-maroon)]">{detail.poCode}</p><h2 className="mt-1 text-xl font-black">{detail.supplierNameSnapshot}</h2><div className="mt-2"><Status value={detail.status} /></div></div><div className="flex flex-wrap items-center justify-end gap-2">
  <button
...
</div></header>{isDetailLoading ? <div className="flex items-center justify-center gap-2 p-10"><LoaderCircle className="animate-spin" size={18} />Loading PO...</div> : <div className="space-y-5 p-5"><div className="grid gap-3 sm:grid-cols-3"><div className="rounded-2xl bg-[var(--color-soft)] p-4"><p className="text-xs text-[var(--color-muted)]">Ordered</p><p className="font-bold">{dateOnly(detail.orderDate)}</p></div><div className="rounded-2xl bg-[var(--color-soft)] p-4"><p className="text-xs text-[var(--color-muted)]">Expected</p><p className="font-bold">{dateOnly(detail.expectedDate)}</p></div><div className="rounded-2xl bg-[var(--color-soft)] p-4"><p className="text-xs text-[var(--color-muted)]">Grand total</p><p className="font-black">{money(detail.grandTotal)}</p></div></div><div className="overflow-x-auto rounded-2xl border"><table className="w-full min-w-[700px] text-left text-sm"><thead className="bg-[var(--color-soft)] text-xs uppercase"><tr><th className="p-3">Item</th><th className="p-3 text-right">Ordered</th><th className="p-3 text-right">Received</th><th className="p-3 text-right">Unit cost</th><th className="p-3 text-right">Line total</th></tr></thead><tbody className="divide-y">{(detail.items || []).map((line) => <tr key={line.id}><td className="p-3"><p className="font-bold">{line.description}</p><p className="text-xs text-[var(--color-muted)]">{line.item?.itemCode || "Unlinked line"}</p></td><td className="p-3 text-right">{Number(line.quantity)}</td><td className="p-3 text-right">{Number(line.receivedQuantity)}</td><td className="p-3 text-right">{money(line.unitCost)}</td><td className="p-3 text-right font-black">{money(line.lineTotal)}</td></tr>)}</tbody></table></div><div className="flex flex-wrap gap-2 pt-2">{detail.status === "DRAFT" ? <><button className="rounded-xl border border-[var(--color-border)] bg-white px-4 py-2.5 text-xs font-bold" onClick={() => { const target = detail; setDetail(null); setEditing(target) }} type="button">Edit draft</button><button className="rounded-xl bg-emerald-600 px-4 py-2.5 text-xs font-bold text-white shadow-sm disabled:opacity-50" disabled={isSaving} onClick={() => changeStatus(detail, "ORDERED")} type="button">Mark as Ordered</button><button className="rounded-xl border border-rose-200 bg-rose-50 px-4 py-2.5 text-xs font-bold text-rose-700 disabled:opacity-50" disabled={isSaving} onClick={() => changeStatus(detail, "CANCELLED")} type="button">Cancel PO</button></> : null}{["ORDERED", "PARTIALLY_RECEIVED"].includes(detail.status) && onNavigate ? <button className="inline-flex items-center gap-2 rounded-xl bg-[var(--color-maroon)] px-4 py-3 text-sm font-bold text-white" onClick={() => onNavigate("receivings")} type="button"><Send size={16} />Open receiving</button> : null}</div></div>}</section></div> : null}
    </div>
  )
}
