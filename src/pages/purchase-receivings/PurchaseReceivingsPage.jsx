...
function Status({ value }) {
  const classes = value === "POSTED" ? "bg-emerald-50 text-emerald-700" : value === "CANCELLED" ? "bg-rose-50 text-rose-700" : "bg-amber-50 text-amber-700"
  return <span className={`rounded-full px-2.5 py-1 text-xs font-black ${classes}`}>{value}</span>
}
...
/><label className="block">
  ...
  />
</label><button className="grid size-10 place-items-center rounded-xl border border-rose-200 text-rose-700 disabled:opacity-30" disabled={form.items.length === 1 || Boolean(selectedPo)} onClick={() => setForm((current) => ({ ...current, items: current.items.filter((_, lineIndex) => lineIndex !== index) }))} type="button"><Trash2 size={16} /></button></div><div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3"><label className="text-xs font-bold">Batch code<input className="mt-1 w-full rounded-xl border px-3 py-2.5 text-sm font-normal" onChange={(event) => updateLine(index, { batchCode: event.target.value })} placeholder="Required to post" value={line.batchCode} /></label><label className="text-xs font-bold">Expiry date (optional)<input className="mt-1 w-full rounded-xl border px-3 py-2.5 text-sm font-normal" onChange={(event) => updateLine(index, { expiryDate: event.target.value })} type="date" value={line.expiryDate} /></label>{!item ? (
...
export default function PurchaseReceivingsPage({ selectedBranch, user }) {
  ...
  return (
    <div className="space-y-5">
      <section className="rounded-3xl border bg-white p-5 shadow-card sm:p-6"><div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-xs font-black uppercase tracking-[0.2em] text-[var(--color-maroon)]">Supply chain</p><h1 className="mt-2 text-2xl font-black">Receiving / Deliveries</h1><p className="mt-1 text-sm text-[var(--color-muted)]">Draft, validate, and post supplier deliveries into the correct branch inventory.</p></div><button className="inline-flex items-center justify-center gap-2 rounded-xl bg-[var(--color-maroon)] px-4 py-3 text-sm font-bold text-white" onClick={() => setEditing({})} type="button"><Plus size={17} />New receiving</button></div></section>
      ...
      <section className="grid gap-3 rounded-3xl border bg-white p-4 shadow-card sm:grid-cols-[1fr_240px]"><label className="relative"><Search className="absolute left-3 top-1/2 -translate-y-1/2 text-[var(--color-muted)]" size={16} /><input className="w-full rounded-xl border py-3 pl-10 pr-3 text-sm" onChange={(event) => { setSearch(event.target.value); setPage(1) }} placeholder="Receiving, invoice, delivery, supplier" value={search} /></label><select className="rounded-xl border px-3 py-3 text-sm" onChange={(event) => { setStatus(event.target.value); setPage(1) }} value={status}><option value="">All statuses</option><option value="DRAFT">Draft</option><option value="POSTED">Posted</option><option value="CANCELLED">Cancelled</option></select></section>
      <section className="overflow-hidden rounded-3xl border bg-white shadow-card">{isLoading ? <div className="flex items-center justify-center gap-2 p-10"><LoaderCircle className="animate-spin" size={18} />Loading deliveries...</div> : receivings.length === 0 ? <div className="p-10 text-center"><Boxes className="mx-auto text-[var(--color-muted)]" size={40} /><p className="mt-3 font-black">No matching receivings</p></div> : <><div className="hidden overflow-x-auto lg:block"><table className="w-full min-w-[960px] text-left text-sm"><thead className="bg-[var(--color-soft)] text-xs uppercase text-[var(--color-muted)]"><tr><th className="px-4 py-3">Receiving</th><th className="px-4 py-3">Supplier / PO</th><th className="px-4 py-3">References</th><th className="px-4 py-3">Status</th><th className="px-4 py-3 text-right">Total</th><th className="px-4 py-3 text-right">Actions</th></tr></thead><tbody className="divide-y">{receivings.map((receiving) => <tr key={receiving.id}><td className="px-4 py-4"><p className="font-black">{receiving.receivingCode}</p><p className="text-xs text-[var(--color-muted)]">{dateOnly(receiving.receivingDate)} · {receiving.branch?.code}</p></td><td className="px-4 py-4"><p>{receiving.supplierNameSnapshot}</p><p className="text-xs text-[var(--color-muted)]">{receiving.purchaseOrder?.poCode || "Standalone"}</p></td><td className="px-4 py-4">{receiving.supplierInvoiceNo || receiving.supplierDeliveryNo || receiving.referenceNo || "—"}</td><td className="px-4 py-4"><Status value={receiving.status} /></td><td className="px-4 py-4 text-right font-black">{money(receiving.grandTotal)}</td><td className="px-4 py-4"><div className="flex justify-end gap-2"><button className="rounded-xl border p-2" onClick={() => openDetail(receiving)} type="button"><Eye size={16} /></button>{receiving.status === "DRAFT" ? <button className="rounded-xl border px-3 py-2 text-xs font-bold" onClick={() => setEditing(receiving)} type="button">Edit</button> : null}{receiving.status === "DRAFT" ? <button className="rounded-xl bg-emerald-600 px-3 py-2 text-xs font-bold text-white" disabled={isSaving} onClick={() => changeStatus(receiving, "POSTED")} type="button">Post stock</button> : null}{receiving.status === "DRAFT" ? <button className="rounded-xl border border-rose-200 px-3 py-2 text-xs font-bold text-rose-700" disabled={isSaving} onClick={() => changeStatus(receiving, "CANCELLED")} type="button">Cancel</button> : null}</div></td></tr>)}</tbody></table></div><div className="grid gap-3 p-4 lg:hidden">{receivings.map((receiving) => <article className="rounded-2xl border border-[var(--color-border)] p-4 shadow-sm" key={receiving.id}><div className="flex items-start justify-between gap-2"><div><p className="font-black text-[var(--color-text-strong)]">{receiving.receivingCode}</p><p className="mt-1 text-sm text-[var(--color-muted)]">{receiving.supplierNameSnapshot}</p></div><Status value={receiving.status} /></div><div className="mt-3 flex items-end justify-between border-t border-[var(--color-border)] pt-3"><p className="text-sm text-[var(--color-muted)]">{dateOnly(receiving.receivingDate)}</p><p className="font-black text-[var(--color-maroon)]">{money(receiving.grandTotal)}</p></div><div className="mt-4 flex flex-wrap gap-2"><button className="flex-1 min-w-[70px] rounded-xl border border-[var(--color-border)] bg-white px-3 py-2 text-xs font-bold transition hover:bg-[var(--color-soft)]" onClick={() => openDetail(receiving)} type="button">View</button>{receiving.status === "DRAFT" ? <><button className="rounded-xl border border-[var(--color-border)] bg-white px-3 py-2 text-xs font-bold transition hover:bg-[var(--color-soft)]" onClick={() => setEditing(receiving)} type="button">Edit</button><button className="rounded-xl bg-emerald-600 px-3 py-2 text-xs font-bold text-white shadow-sm transition hover:bg-emerald-700 disabled:opacity-50" disabled={isSaving} onClick={() => changeStatus(receiving, "POSTED")} type="button">Post stock</button><button className="rounded-xl border border-rose-200 bg-rose-50 px-3 py-2 text-xs font-bold text-rose-700 transition hover:bg-rose-100 disabled:opacity-50" disabled={isSaving} onClick={() => changeStatus(receiving, "CANCELLED")} type="button">Cancel</button></> : null}</div></article>)}</div></>}
        <div className="flex items-center justify-between border-t p-4"><p className="text-sm text-[var(--color-muted)]">Page {pagination.page || page} of {totalPages}</p><div className="flex gap-2"><button className="rounded-xl border p-2 disabled:opacity-30" disabled={page <= 1} onClick={() => setPage((value) => Math.max(1, value - 1))} type="button"><ChevronLeft size={18} /></button><button className="rounded-xl border p-2 disabled:opacity-30" disabled={page >= totalPages} onClick={() => setPage((value) => value + 1)} type="button"><ChevronRight size={18} /></button></div></div></section>
      {editing ? <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-950/55 p-3 sm:p-6"><section className="mx-auto max-w-7xl rounded-3xl bg-white p-5 shadow-2xl"><div className="mb-5 flex justify-between"><h2 className="text-xl font-black">{editing.id ? `Edit ${editing.receivingCode}` : "New purchase receiving"}</h2><button className="rounded-xl border p-2" onClick={() => setEditing(null)} type="button"><X size={18} /></button></div><ReceivingForm catalogItems={catalogItems} initial={editing} isSaving={isSaving} onClose={() => setEditing(null)} onSave={save} purchaseOrders={purchaseOrders} suppliers={suppliers} /></section></div> : null}
      {detail ? <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-950/55 p-3 sm:p-6"><section className="mx-auto max-w-5xl rounded-3xl bg-white shadow-2xl"><header className="flex items-start justify-between border-b p-5"><div><p className="text-xs font-black text-[var(--color-maroon)]">{detail.receivingCode}</p><h2 className="mt-1 text-xl font-black">{detail.supplierNameSnapshot}</h2><div className="mt-2"><Status value={detail.status} /></div></div><div className="flex flex-wrap items-center justify-end gap-2">
  <button
    className="rounded-xl border border-[var(--color-border)] bg-white px-3 py-2 text-xs font-bold"
    onClick={() =>
...
</div></header>{isDetailLoading ? <div className="flex items-center justify-center gap-2 p-10"><LoaderCircle className="animate-spin" size={18} />Loading receiving...</div> : <div className="space-y-5 p-5"><div className="grid gap-3 sm:grid-cols-3"><div className="rounded-2xl bg-[var(--color-soft)] p-4"><p className="text-xs text-[var(--color-muted)]">Purchase order</p><p className="font-bold">{detail.purchaseOrder?.poCode || "Standalone"}</p></div><div className="rounded-2xl bg-[var(--color-soft)] p-4"><p className="text-xs text-[var(--color-muted)]">Reference</p><p className="font-bold">{detail.supplierInvoiceNo || detail.supplierDeliveryNo || detail.referenceNo || "—"}</p></div><div className="rounded-2xl bg-[var(--color-soft)] p-4"><p className="text-xs text-[var(--color-muted)]">Grand total</p><p className="font-black">{money(detail.grandTotal)}</p></div></div><div className="space-y-3">{(detail.items || []).map((line) => <article className="rounded-2xl border p-4" key={line.id}><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><p className="font-black">{line.item?.itemCode} · {line.item?.itemName}</p><p className="mt-1 text-sm text-[var(--color-muted)]">{line.description}</p><p className="mt-1 text-xs">Batch {line.batchCode || "not set"} · Qty {Number(line.quantityReceived)} · {money(line.unitCost)} each</p></div><p className="font-black">{money(line.lineTotal)}</p></div>{(line.serials || []).length ? <div className="mt-3 flex flex-wrap gap-2">{line.serials.map((serial) => <span className="rounded-lg bg-[var(--color-soft)] px-2 py-1 text-xs font-bold" key={serial.id}>{serial.serialNumber}</span>)}</div> : null}</article>)}</div>{detail.status === "DRAFT" ? <div className="flex flex-wrap gap-2"><button className="inline-flex items-center gap-2 rounded-xl bg-emerald-600 px-4 py-3 text-sm font-bold text-white" disabled={isSaving} onClick={() => changeStatus(detail, "POSTED")} type="button"><PackagePlus size={16} />Post to inventory</button><button className="inline-flex items-center gap-2 rounded-xl border px-4 py-3 text-sm font-bold" onClick={() => setEditing(detail)} type="button"><CheckCircle2 size={16} />Review/edit draft</button></div> : null}</div>}</section></div> : null}
    </div>
  )
}
