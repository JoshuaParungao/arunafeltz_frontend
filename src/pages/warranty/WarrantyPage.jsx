...
function statusTone(status) {
  ...
  if (status === "SENT_TO_SUPPLIER")
    return "bg-violet-100 dark:bg-violet-900/40 text-violet-800 dark:text-violet-300"
...
function StatusBadge({ status }) {
  return (
    <span className={`inline-flex items-center rounded-full px-2.5 py-1 text-xs font-black ${statusTone(status)}`}>
      {formatStatus(status)}
    </span>
  )
}
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center overflow-y-auto bg-black/60 p-3 sm:p-6 backdrop-blur-xs">
      <section
        aria-label={title}
        aria-modal="true"
        className={`my-auto w-full overflow-hidden rounded-3xl border border-[var(--color-border)] bg-[var(--color-card)] text-[var(--color-text)] shadow-2xl ${
