    ...
    <div className="space-y-5">
      <section className="rounded-3xl border border-[var(--color-border)] bg-[var(--color-card)] p-5 shadow-card sm:p-6">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          ...
          <div className="flex flex-wrap gap-2"><button className="inline-flex items-center gap-2 rounded-xl border border-[var(--color-border)] px-4 py-2.5 text-sm font-bold hover:bg-[var(--color-soft)]" disabled={isLoading} onClick={refresh} type="button"><RefreshCw className={isLoading ? "animate-spin" : ""} size={16} /> Refresh</button>{canCreate ? <button className="inline-flex items-center gap-2 rounded-xl bg-[var(--color-maroon)] px-4 py-2.5 text-sm font-bold text-white transition hover:opacity-90" onClick={() => setShowCreate(true)} type="button"><Plus size={17} /> Receive claim</button> : null}</div>
        </div>
        <div className="mt-5 grid gap-3 sm:grid-cols-3">{[["Active on this page", pageSummary.active, ClipboardCheck], ["With supplier", pageSummary.supplier, ShieldCheck], ["Ready to release", pageSummary.release, PackageCheck]].map(([label, value, Icon]) => <div className="rounded-2xl bg-[var(--color-soft)] p-4 border border-[var(--color-border)]" key={label}><Icon className="text-[var(--color-maroon)]" size={18} /><p className="mt-3 text-2xl font-black text-[var(--color-text-strong)]">{value}</p><p className="text-xs font-bold text-[var(--color-muted)]">{label}</p></div>)}</div>
      </section>
