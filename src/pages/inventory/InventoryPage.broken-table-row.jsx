...
const canAdjustStock = ["SUPER_OWNER", "ADMIN"].includes(user?.role)
                  ...
                  {items.map((item) => (
                    <tr key={item.id} className="align-top transition hover:bg-[var(--color-soft)]">
                      ...
                      <td className="whitespace-nowrap px-4 py-4 text-[var(--color-muted)]">
                        ...
                        <td className="px-5 py-4">                           <div className="flex flex-wrap gap-2">                             {canAdjustStock ? (                               <button                                 className="inline-flex items-center justify-center rounded-2xl border border-[#7A1F2B] bg-white px-4 py-2 text-xs font-black text-[#7A1F2B] transition hover:bg-[#F4F1EC]"                                 onClick={() => openAdjustModal(item)}                                 type="button"                               >                                 Adjust stock                               </button>                             ) : null}                              {canRequestFromViewedBranch ? (                               <button                                 className="inline-flex items-center justify-center rounded-2xl border border-[#7A1F2B] bg-white px-4 py-2 text-xs font-black text-[#7A1F2B] transition hover:bg-[#F4F1EC] disabled:cursor-not-allowed disabled:border-gray-300 disabled:text-gray-400 disabled:opacity-60"                                 disabled={Number(item.quantityAvailable || 0) <= 0}                                 onClick={() => openRequestModal(item)}                                 type="button"                               >                                 Add to request                               </button>                             ) : null}                              {!canAdjustStock && !canRequestFromViewedBranch ? (                               <span className="text-xs font-semibold text-[var(--color-muted)]">Own branch</span>                             ) : null}                           </div>                         </td>
                      ...
                      <td className="whitespace-nowrap px-4 py-4 text-[var(--color-muted)]">
                        ...
                        <div className="flex flex-wrap gap-2">                           {canAdjustStock ? (                             <button                               className="inline-flex items-center justify-center rounded-2xl border border-[#7A1F2B] bg-white px-4 py-2 text-xs font-black text-[#7A1F2B] transition hover:bg-[#F4F1EC]"                               onClick={() => openAdjustModal(item)}                               type="button"                             >                               Adjust stock                             </button>                           ) : null}                            {canRequestFromViewedBranch ? (                             <button                               className="inline-flex items-center justify-center rounded-2xl border border-[#7A1F2B] bg-white px-4 py-2 text-xs font-black text-[#7A1F2B] transition hover:bg-[#F4F1EC] disabled:cursor-not-allowed disabled:border-gray-300 disabled:text-gray-400 disabled:opacity-60"                               disabled={Number(item.quantityAvailable || 0) <= 0}                               onClick={() => openRequestModal(item)}                               type="button"                             >                               Add to request                             </button>                           ) : null}                            {!canAdjustStock && !canRequestFromViewedBranch ? (                             <span className="text-xs font-semibold text-[var(--color-muted)]">Own branch</span>                           ) : null}                         </div>
                    </tr>
                  ))}
                </tbody>
              </table>
