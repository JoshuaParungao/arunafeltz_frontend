...
import { AlertCircle, PackageSearch, RefreshCw, Search, SlidersHorizontal } from "lucide-react"
...
export default function InventoryPage({ selectedBranch, user }) {
  ...
  const openAdjustModal = async (item) => {
    ...
    try {
      const response = await getInventoryBatches({
        branchId: viewingBranchId || item.branch?.id,
        itemId: item.id,
        limit: 50,
      })
  ...
  const submitBulkStockRequest = async () => {
    if (!viewingBranchId) {
      setRequestMessage("Choose a branch first.")
      return
    }
    ...
    const invalidItem = bulkRequestItems.find((item) => {
      ...
      return !Number.isFinite(quantity) || quantity <= 0 || quantity > availableQuantity
    })
  ...
  const submitStockAdjustment = async (event) => {
    ...
    if (!adjustItem || !adjustBatchId) {
      setAdjustMessage("Choose a stock batch first.")
      return
    }
    ...
    try {
      await createStockAdjustment({
        branchId: viewingBranchId || adjustItem.branch?.id,
                    ...
                    <tr key={item.id} className="align-top transition hover:bg-[var(--color-soft)]">
                      ...
                      <td className="whitespace-nowrap px-4 py-4">
                        <div className="flex flex-wrap gap-2">
                          {canAdjustStock ? (
                            <button
                              className="inline-flex items-center justify-center rounded-2xl border border-[var(--color-border)] bg-white px-4 py-2 text-xs font-black text-[var(--color-text-strong)] transition hover:border-[#7A1F2B] hover:text-[#7A1F2B]"
                          ...
                          {!canAdjustStock && !canRequestFromViewedBranch ? (
                            <span className="text-xs font-semibold text-[var(--color-muted)]">Own branch</span>
                          ) : null}
                        </div>
                      </td>
                    </tr>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="w-full max-w-xl rounded-3xl bg-white p-6 shadow-2xl">
            <div className="flex items-start justify-between gap-4">
              <div>
                <p className="text-xs font-black uppercase tracking-[0.2em] text-[#7A1F2B]">
                  Inventory
                </p>
              ...
              <button
                className="rounded-2xl border border-[var(--color-border)] px-4 py-2 text-sm font-bold text-[var(--color-muted)]"
            ...
            <div className="mt-5 rounded-2xl bg-[var(--color-soft)] p-4">
              <p className="font-black text-[var(--color-text-strong)]">{adjustItem.itemName}</p>
              <p className="mt-1 text-sm font-semibold text-[var(--color-muted)]">{adjustItem.itemCode}</p>
            </div>

            <form className="mt-5 space-y-4" onSubmit={submitStockAdjustment}>
              <label className="block">
                <span className="text-sm font-black text-[var(--color-text-strong)]">Batch</span>
                <select
                  className="mt-2 w-full rounded-2xl border border-[var(--color-border)] px-4 py-3 text-sm font-semibold outline-none focus:border-[#7A1F2B]"
                  disabled={isAdjusting}
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <label className="block">
                  <span className="text-sm font-black text-[var(--color-text-strong)]">Type</span>
                  <select
                    className="mt-2 w-full rounded-2xl border border-[var(--color-border)] px-4 py-3 text-sm font-semibold outline-none focus:border-[#7A1F2B]"
                    disabled={isAdjusting}
                ...
                <label className="block">
                  <span className="text-sm font-black text-[var(--color-text-strong)]">Quantity</span>
                  <input
                    className="mt-2 w-full rounded-2xl border border-[var(--color-border)] px-4 py-3 text-sm font-semibold outline-none focus:border-[#7A1F2B]"
                    disabled={isAdjusting}
                    min="1"
              ...
              <label className="block">
                <span className="text-sm font-black text-[var(--color-text-strong)]">Reference No.</span>
                <input
                  className="mt-2 w-full rounded-2xl border border-[var(--color-border)] px-4 py-3 text-sm font-semibold outline-none focus:border-[#7A1F2B]"
                  disabled={isAdjusting}
              ...
              <label className="block">
                <span className="text-sm font-black text-[var(--color-text-strong)]">Reason</span>
                <textarea
                  className="mt-2 min-h-28 w-full rounded-2xl border border-[var(--color-border)] px-4 py-3 text-sm font-semibold outline-none focus:border-[#7A1F2B]"
                  disabled={isAdjusting}
              ...
              <div className="flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
                <button
                  className="rounded-2xl border border-[var(--color-border)] px-5 py-3 text-sm font-black text-[var(--color-muted)]"
                  disabled={isAdjusting}
                  onClick={() => closeAdjustModal()}
                  type="button"
                >
                ...
                <button
                  className="rounded-2xl bg-[#7A1F2B] px-5 py-3 text-sm font-black text-white transition hover:bg-[#641824] disabled:cursor-not-allowed disabled:opacity-60"
                  disabled={isAdjusting}
                  type="submit"
                >
