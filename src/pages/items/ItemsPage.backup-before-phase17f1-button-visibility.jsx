    ...
    <div className="fixed inset-0 z-50 grid place-items-center bg-black/40 px-4 py-6">
      <section className="w-full max-w-2xl rounded-3xl border border-[var(--color-border)] bg-white shadow-card">
        ...
        <div className="flex flex-col-reverse gap-3 border-t border-[var(--color-border)] p-5 sm:flex-row sm:justify-end">
          ...
          <button
            className="rounded-2xl bg-[var(--color-accent)] px-5 py-3 text-sm font-bold text-white transition hover:bg-[var(--color-accent-hover)] disabled:cursor-not-allowed disabled:opacity-60"
            disabled={isSaving}
            onClick={onSave}
            type="button"
          >
...
function ItemMobileCard({ canManagePrices, canViewCost, item, onEditPrices }) {
  return (
    <article className="rounded-3xl border border-[var(--color-border)] bg-white p-4 shadow-sm">
      ...
      <div className="mt-4 grid grid-cols-2 gap-3 text-sm">
        {PRICE_FIELDS.slice(0, 3).map((field) => (
          <div key={field.key}>
      ...
      {canManagePrices ? (
        <button
          className="mt-4 inline-flex w-full items-center justify-center gap-2 rounded-2xl bg-[var(--color-accent)] px-4 py-3 text-sm font-bold text-white transition hover:bg-[var(--color-accent-hover)]"
                    ...
                    {items.map((item) => (
                      <tr key={item.id} className="align-top transition hover:bg-[var(--color-soft)]">
                        ...
                        {canManagePrices ? (
                          <td className="whitespace-nowrap px-4 py-4">
                            <button
                              className="inline-flex items-center justify-center gap-2 rounded-2xl bg-[var(--color-accent)] px-4 py-2 text-xs font-bold text-white transition hover:bg-[var(--color-accent-hover)]"
